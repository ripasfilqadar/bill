...
if [[ -z `dpkg -l | grep elasticsearch` ]]; then
  wget --no-check-certificate https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-2.4.0.deb
  sudo dpkg -i elasticsearch-2.4.0.deb && rm elasticsearch-2.4.0.deb
fi
